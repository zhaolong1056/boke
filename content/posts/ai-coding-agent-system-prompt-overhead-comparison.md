---
title: "你的 AI 编程助手在烧 Token，而你浑然不知"
date: 2026-07-14
tags:
  - AI
  - 编程工具
...
> 同一行 bug，Claude Code 用 23000 个 token 修好，Gemini 烧了 118500 个[^1]。差距不在模型聪明程度，而在"系统提示"这个你看不见的黑洞。

[^1]: Gemini 2.5 Pro 数据来自开发者 Lars de Ridder 的 Context Lens 实测，与 Claude Code 做的是完全相同的 Express.js `res.send(null)` bug。118.5K 是 Gemini 在 Agent 模式下多轮工具调用的累计消耗。
...
这对你的钱包是实打实的影响。API 模式下，每一条消息都要为 context 中的所有 token 付费。一个 20K token 的 system prompt，聊 10 轮就是至少 200K 的输入开销——而且这些 token 不是花在你的任务上，是花在"告诉模型怎么当助手"上。
...
Claude Code 给了开发者几种管理 context 的手段：
...
换句话说，Claude Code 的系统提示非常"重"，70% 是工具定义。但子代理机制巧妙地绕过了这个问题——把脏活累活扔到独立 context 里，主对话保持清爽。

这里有个重要的平衡器：**Prompt Caching**。Anthropic 的提示词缓存让重复输入部分（系统提示词、工具定义）只需 10% 的费用——缓存读 $0.30/M token vs 常规输入 $3/M token。

也就是说，虽然 Claude Code 每轮对话都要重发 20K token 的系统提示，但如果你连续对话，从第二轮起这 20K 只花正常 10% 的钱。这是 Claude Code 重度用户的账单没有"线性爆炸"的核心原因。

但缓存不是万能药。第一轮对话没有缓存命中；跨 session 的缓存会在 5 分钟不活跃后过期；而且缓存不计入 context window 的压缩——你的可用空间还是被占着，只是便宜了。
...
| 数字 | 含义 | Token 数 |
|------|------|---------|
| 广告标称 | GPT-5.5 API context window | 1,050,000 |
...
管理 context 的方式和 Claude Code 类似：
...
Codex 的"名义 105 万→实际 25.8 万"落差暴露了一个行业通病：厂商宣传的最大 context window 和你能用的 context window 是两回事。系统开销 + 输出预留 = 你的真实可用空间远小于广告数字。
...
OpenCode 是一个极端案例，但它说明了一个规律：**工具生态越丰富，隐形税收越高。** 每个 Skill、每个 Subagent、每个 MCP 工具，都在你不注意的时候往 context 里塞了几百到几千个 token。这些"为了你好"的能力，最终都变成了你账单上的固定成本。

作为对比，Aider 走的是完全相反的路——没有子代理、没有 Skills 生态、没有 MCP。它的平均每任务消耗约 105K token，而 Claude Code 做同样的任务要 479K（MorphLLM 跨 3 个代码库 6 个月的实测）。Aider 少了 4.2 倍的 token 开销，但也少了 Claude Code 的分层架构、自动记忆和子代理能力。

这就引出了一个尴尬的取舍：**Agent 能力越强，系统提示越重。** Aider 省下来的 token 是用功能换的。
...
但 Cursor 也有自己的问题。同一 benchmark 中它完成了 188K token 的消耗，是 Claude Code 的 5.5 倍（来源：tech-insider.org 的对比测试）。这不是系统提示开销，而是因为它的 context 构建策略在每一步都"撒网更广"。

换句话说，没有 system prompt 包袱 ≠ 更省 token。Context 构建策略的差异可以比 system prompt 本身产生更大的开销差异。
...
系统提示开销最终体现在你的账单上。下面是用同一套任务 Profile（Bug 修复 25 次 API 调用、400K 累计输入；Feature 100 次、2M 输入）跑出来的实际成本：

| Agent / 模型 | 一个 Bug Fix | 一个 Feature | 20 个 Features/月 |
|-------------|------------|------------|-----------------|
| DeepSeek V4 Pro | $0.05 | $0.21 | $4.20 |
| Kimi K2.6 | $0.18 | $0.80 | $16.00 |
| Codex CLI (GPT-5.3) | $0.37 | $1.54 | $30.80 |
| **Cursor / Claude Code (Sonnet 4.6)** | **$0.54** | **$2.28** | **$45.60** |
| Claude Code (Opus 4.8) | $0.90 | $3.80 | $76.00 |

数据来源：MorphLLM AI Coding Costs（2026 年 6 月），按照 Sonnet 4.6 API 定价 $3/M in + $15/M out 计算。

这些成本的前提是你自带 API Key（BYOK）。如果走订阅制，Cursor Pro 月 $20、Claude Code Max 20x 约 $200、Codex Plus $20+ 超额——费用结构不同但量级相当。Microsoft 曾因 Claude Code 达到 **$2,000/工程师/月** 而终止了企业许可（Windows Central 报道）。

**关键结论：** 系统提示开销在单体任务上看似微不足道（0.54 和 0.37 差不了多少），但乘以每天的迭代次数，月差可达上百美元。而且——模型越贵，系统提示开销造成的浪费越肉疼。
